...
//初始化人脸对齐器
//static seeta::FaceAlignment point_detector(tAlignModelPath.c_str());
...
//人脸框
jfieldID tLeftField = NULL, tRightField = NULL, tTopField = NULL, tBottomField = NULL;

//特征点数组,眼睛鼻子嘴巴
jfieldID tLandmarkField = NULL;

//特征数组
jfieldID tFeaturesField = NULL;
...
JNIEXPORT void JNICALL Java_utryface_UtryFace_test(JNIEnv *env, jobject obj)
{
	jclass cls;
	jfieldID fid;
	jint i;
	...
	//printf("Java_utryface_UtryFace_test, i=%d\n", i); 
	(*env).SetIntField(obj, fid, i * 4);//将i*2值设置到fid的ID中，即java中变量i的值
...
{ 
	...
	//time_t t2;
	//int tTime2 = time(&t2);
	//LOGD("DetectFacesByte, 总耗时:%d秒", tTime2 - tTime0);
	
	LOGD("DetectFacesByte, done\n"); 
