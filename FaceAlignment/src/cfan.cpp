...
void CCFAN::FacialPointLocate(const unsigned char *gray_im, int im_width, int im_height, seeta::FaceInfo face_loc, float *facial_loc)
{
  ...
  int extend_lx = max(floor(left_x - extend_factor*bbox_w), double(0));
  int extend_rx = min(floor(right_x + extend_factor*bbox_w), double(im_width - 1));
  int extend_ly = max(floor(left_y - (extend_factor - extend_revised_y)*bbox_h), double(0));
  int extend_ry = min(floor(right_y + (extend_factor + extend_revised_y)*bbox_h), double(im_height - 1));
